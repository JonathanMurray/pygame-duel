...
def initPygame():
	...
	pygame.display.set_caption('* JM Game *')
	GRAY = (100,100,100)
	BLACK = (0, 0, 0)
	WHITE = (255, 255, 255)
	YELLOW = (255,255,0)
...
class Player:
	def __init__(self, name, x, y, color, movementKeys, shootKey, missileKey, direction):
		...
		self.moveSpeed = 1.6
		self.gunPos = (self.x + self.w/2 + self.direction[0] * self.w / 4, self.y + self.h/2 + self.direction[1] * self.h / 4)
	...
	def move(self, dx, dy):
		...
		if self.x + dx > 0 and self.x + self.w + dx < windowWidth and self.y + dy > 20 and self.y + self.h + dy < windowHeight:
			self.x += dx
			self.y += dy
			self.gunPos = (int(self.x + self.w/2 + self.direction[0] * self.w / 4), int(self.y + self.h/2 + self.direction[1] * self.h / 4))
			self.x = int(self.x)
			self.y = int(self.y)
	...
	def shoot(self):
		...
		bullet = Bullet(self.gunPos[0], self.gunPos[1], self.color, self.bulletRadius, direction, self.bulletSpeed, self, bulletRandomness)
